...
//! propagate that spelling change when all reachable providers agree. Inference also lets valid
...
use ty_python_semantic::{HasType, SemanticModel, ValueProviders, binding_type};
            ...
            let direct = resolved_parent.is_some_and(|parent| {
                imported.is_some_and(|module| {
                    ...
                    alias.name.as_str() == name.last_component()
                        ...
                        && (self
                            .model
                            .module_global_providers(parent, alias.name.as_str())
                            .is_none()
                            || file_to_module(
                                self.db,
...
fn module_export_decision<'db>(
    ...
    // Explicit re-exports form a graph. Follow direct providers until their binding policy is
    ...
    let decision = model
        .module_global_providers(module, name)
        .map_or(Err(()), |providers| {
            provider_decision(&providers, |definition| {
...
impl SemanticPass<'_, '_> {
    fn name(&mut self, name: &ast::ExprName) {
        ...
        let mut decision = provider_decision(load.providers(), |definition| {
    ...
    fn attribute(&mut self, attribute: &ast::ExprAttribute) -> TraversalSignal {
        ...
        let providers = self
            .model
            .module_global_providers(receiver, attribute.attr.as_str());
        if providers.is_none() && !rule.package && rule.old_name.parent() != rule.new_name.parent()
        {
            let decision = self.module_expression_decision(attribute, &new);
            self.apply(attribute.range, decision);
            return TraversalSignal::Skip;
        }
        let decision = if let Some(providers) = providers {
...
fn provider_decision<'db>(
    providers: &ValueProviders<'db>,
    mut replacement_for: impl FnMut(Definition<'db>) -> Result<Option<String>, ()>,
) -> Decision {
    if providers.has_unrepresented_provider() || providers.may_be_deleted() {
        return Err(());
    }
    let mut definitions = providers.definitions();
    ...
    if definitions.any(|definition| replacement_for(definition) != Ok(replacement.clone())) {
        return Err(());
    }
    match replacement {
...
mod tests {
    ...
    #[test]
    fn conflicting_place_load_providers_are_omitted() {
        let db = test_db(&[
            ("/a/__init__.py", ""),
            ("/a/x.py", ""),
            ("/b/__init__.py", ""),
