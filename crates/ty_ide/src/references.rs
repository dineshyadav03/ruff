...
//!
//! For shared-definition matching, an occurrence is represented by its set of
//! semantic definitions. Two occurrences share a definition when these sets
//! overlap. This relationship is symmetric and does not depend on which binding
//! reaches a load. For example, in `x = 1; print(x)`, the assignment introduces
//! a definition shared by both occurrences.
//!
//! "References" functionality follows use-def relationships:
//!
...
//!   later `print(x)`.
//!
//! Rename and document highlights use shared-definition matching for every
//! occurrence instead of partitioning results by use-def relationships.
//!
